"""ARCs5 Task Browser — RULE LEARNER V6"""
...
def build_learned_rule_text(
    ...
    if not generalized_stories:
        return "\n".join([
            ...
            "LEARNED RULE",
            "=" * 72,
            "No learned rule could be built because no generalized",
            "train-pair stories were available.",
        ])

    try:
        learned_rule = learn_rule(generalized_stories)

        return format_learned_rule(
            learned_rule,
            max_candidates=3,
        )
    ...
    def show_current_task(self) -> None:
        ...
        if train_analyses:
            ...
            learned_rule_panel = AnalysisTextPanel(
                self.task_content,
                title="LEARNED RULE",
                text=build_learned_rule_text(train_analyses),
